package com.jnu.student2021100175.data; // 声明包名
...
public class Spriter { // 创建 Spriter 类
    ...
    public Spriter(Context context,int i) { // Spriter 类的构造函数，接收 Context 类型的参数
        ...
        if(i == 1)
        {
            bitmap = ((BitmapDrawable) context.getResources().getDrawable(R.drawable.book_1)).getBitmap();
        }
        if(i == 2)
        {
            bitmap = ((BitmapDrawable) context.getResources().getDrawable(R.drawable.book_2)).getBitmap();
        }
        if(i == 3)
        {
